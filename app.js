...
app.view('poll_submission', async ({ ack, body, view, client }) => {
    ...
    if (parsedQuestions.length === 0) { // Message-only post
        ...
        for (const conversationId of conversationIds) {
            ...
            } catch (error) { console.error(`Failed to send message-only survey to ${conversationId}`, error); }
        }
...
app.command('/groups', async ({ ack, body, client }) => {
    ...
    try {
        await client.views.open({
            trigger_id: body.trigger_id,
            view: { type: 'modal', callback_id: 'manage_groups_view', title: { type: 'plain_text', text: 'Manage User Groups' }, blocks: [{ type: 'section', text: { type: 'mrkdwn', text: 'Create a new group to easily send surveys to the same people.' } }, { type: 'actions', elements: [{ type: 'button', text: { type: 'plain_text', text: 'Create New Group' }, style: 'primary', action_id: 'create_group_button' }] }] }
        });
    } catch (error) { console.error(error); }
});
...
app.view('create_group_submission', async ({ ack, body, view, client }) => {
    ...
    if (!groupName || memberIds.length === 0) {
        await ack({ response_action: 'errors', errors: { group_name_block: 'Group name cannot be empty.', group_members_block: 'Please select at least one member.' } });
        return;
    }
    await ack();
